...
const f1Calendar2026 = [
    ...
    { round: 7, name: "Canadian Grand Prix", hasSprint: true, date: "2026-05-25T01:30:00+05:30", circuit: "Circuit Gilles-Villeneuve", country: "Canada", lat: 45.5017, lon: -73.5228, trackDetails: { length: "4.361 km", laps: 70, corners: 14, firstGP: 1978, record: "1:13.078" }, sessions: { fp1: "2026-05-22T23:00:00+05:30", sprintQuali: "2026-05-23T03:00:00+05:30", sprint: "2026-05-23T21:30:00+05:30", quali: "2026-05-24T01:30:00+05:30", race: "2026-05-25T01:30:00+05:30" } },
...
app.get('/api/next-race', async (req, res) => {
    ...
    try {
        ...
        if (apiNext && apiNext.round && !apiNext.message) {
            ...
            const mapped = {
                ...
                date: apiNext.all_sessions?.race || apiNext.session_time,
    ...
    const now = new Date();
    const seasonCalendar = await getSeasonCalendar();
    const next = findUpcomingRace(seasonCalendar, now);
...
app.post('/predict', authenticateToken, async (req, res) => {
    ...
    const now = new Date();
    const seasonCalendar = await getSeasonCalendar();
    const currentRace = findUpcomingRace(seasonCalendar, now);
...
app.get('/api/live/weather', async (_, res) => {
    ...
    try {
        ...
        if (!next) {
            const now = new Date();
            const seasonCalendar = await getSeasonCalendar();
            const localNext = findUpcomingRace(seasonCalendar, now);
            next = localNext;
        }
        ...
        res.json({
            AirTemp: String(c.temperature_2m),
            TrackTemp: String(Math.round(c.temperature_2m * 1.15)),
            Humidity: String(c.relative_humidity_2m),
            WindSpeed: String(c.wind_speed_10m),
